...
class VehicleController(Node):
    def __init__(self):
        ...
        # var to store previous errors and steering angle
    ...
    def main_timer_callback(self):
        ...
        if self.have_goal_pose and self.have_vehicle_pose:
            ...
            # get new change in angle
            if self.speed > 0:
                change_in_steering_ang = self.Kp * (heading_error_rad - self.error_m1) + self.Ki*heading_error_rad + self.Kd*(heading_error_rad - 2*self.error_m1 + self.error_m2) 
                self.error_m2 = self.error_m1
                self.error_m1 = heading_error_rad

                self.current_steering = self.current_steering + change_in_steering_ang
